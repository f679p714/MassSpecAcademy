...
Quadrupoles use four parallel charged rods that are connected, diagonally, in pairs. A potential of (U+Vcosωt) is applied to one pair, while a potential of -(U+ωt) applied to the second pair. Here Vcosωt represents an alternating RF voltage and U represents a DC voltage ("DC offset"). If this seems comlicated, just know that the applied potentials create orthogonal electrical fields -- one in the x-plane, and one in the y-plane -- which interact differently with ions of different m/z. 
